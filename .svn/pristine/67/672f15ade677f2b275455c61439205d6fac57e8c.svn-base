 ...
 *
 * Last modified 10/21/16 10:56 AM
 */
...
public class CustomCalendarView extends LinearLayout {
    ...
    private int currentDayOfMonthBackground;

    private int currentMonthIndex = 0;
    private boolean isOverflowDateVisible = true;


    private DayView viewDateOffMonthYear = null;
    ...
    private void getAttributes(AttributeSet attrs) {
        ...
        currentDayOfMonthBackground = typedArray.getColor(R.styleable.CustomCalendarView_currentDayOfMonthColor, getResources().getColor(R.color.color_gray));
        typedArray.recycle();
    }

    private void initializeCalendar() {
        final LayoutInflater inflate = (LayoutInflater) mContext.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        view = inflate.inflate(R.layout.roboto_calendar_layout, this, true);
        ...
        previousMonthButton.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                markDayAsSelectedDayOfYear(currentCalendar.getTime());
            }
        });
    ...
    private void initializeTitleLayout() {
        ...
        String dateText = String.valueOf(new DateFormatSymbols(locale).getShortMonths()[currentCalendar.get(Calendar.MONTH)]);
        ...
        dateTitle.setText(String.valueOf(dateText + " " + currentCalendar.get(Calendar.YEAR)));
        dateTitle.setTextColor(calendarTitleTextColor);
    ...
    private void initializeWeekLayout() {
        ...
        for (int i = 1; i < weekDaysArray.length; i++) {
            ...
            if(dayOfTheWeekString.length() > 4){
                dayOfTheWeekString = dayOfTheWeekString.substring(0, 4).toUpperCase();
            }
    ...
    public void markDayAsCurrentDay(Calendar calendar) {
        ...
        if (calendar != null && isToday(calendar)) {
            viewDateOffMonthYear = getDayOfMonthText(calendar);
            viewDateOffMonthYear.setTextColor(currentDayOfMonth);
            viewDateOffMonthYear.setBackgroundColor(currentDayOfMonthBackground);
            AppCore.showLog("markDayAsCurrentDay--------viewDateOffMonthYear----- : " + String.valueOf(viewDateOffMonthYear.getTag()));
        }
        if(viewDateOffMonthYear != null){
            AppCore.showLog("markDayAsCurrentDay--------viewDateOffMonthYear----- 1: " + String.valueOf(viewDateOffMonthYear.getTag()));
        }
    }
    ...
    public void markDayAsSelectedDay(Date currentDate) {
        ...
        viewDateOffMonthYear = getDayOfMonthText(currentCalendar);
        viewDateOffMonthYear.setBackgroundColor(selectedDayBackground);
        viewDateOffMonthYear.setTextColor(selectedDayTextColor);

        AppCore.showLog("markDayAsSelectedDay--------viewDateOffMonthYear----- : " + String.valueOf(viewDateOffMonthYear.getTag()));
    }

    public void markDayAsSelectedDayOfYear(Date currentDate) {
        if(viewDateOffMonthYear == null) {
            return;
        }
        ...
        DayView viewDate = getDayOfMonthText(currentCalendar);

        if(viewDate == null){
            return;
        }

        if(String.valueOf(viewDate.getTag()).equals(String.valueOf(viewDateOffMonthYear.getTag()))){
            viewDateOffMonthYear.setBackgroundColor(selectedDayBackground);
            viewDateOffMonthYear.setTextColor(selectedDayTextColor);

            AppCore.showLog("markDayAsSelectedDayOfYear--------viewDateOffMonthYear----- : " + String.valueOf(viewDateOffMonthYear.getTag()));
        }

        AppCore.showLog("markDayAsSelectedDayOfYear--------viewDateOffMonthYear-----1 : " + String.valueOf(viewDateOffMonthYear.getTag()));
    }
